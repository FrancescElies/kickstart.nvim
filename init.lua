...
require('lazy').setup({
    ...
    config = function()
      ...
      local function open_with_default_os_app(_, map)
        ...
        map('n', 'go', function(_)
          ...
          vim.ui.open(path)
        end, { desc = '[g]o [o]pen default OS App' })
      ...
      vim.keymap.set('n', '<leader>gs', builtin.git_status, { desc = '[s]witch git [s]tatus' })
      vim.keymap.set('n', 'gs', builtin.git_status, { desc = '[s]witch git [s]tatus' })
  ...
  change_detection = {
    enabled = true, -- auto check config file changes and reload
    notify = false,
  },
  ui = {
