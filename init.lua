...
do
  ...
  vim.keymap.set('n', '<leader>si', builtin.git_status, { desc = '[s]earch git [i]ndex' })
...
do
  ...
  vim.keymap.set({ 'n', 'v' }, '<localleader>f', function() require('conform').format { async = true } end, { desc = '[F]ormat buffer' })
end
